...
import { LineAreaChart, DonutChart, BarChart } from "@/components/design/charts";
...
// ── PageHeader ────────────────────────────────────────────────
function PageHeader({ range, onRangeChange }: { range: Range; onRangeChange: (r: Range) => void }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
      ...
      <div style={{ display: "flex", gap: 4, padding: 4, background: "var(--bg-elevated)", borderRadius: 999, flexWrap: "wrap" }}>
        {RANGES.map((r) => (
          <button
            key={r} type="button" onClick={() => onRangeChange(r)}
            style={{
              padding: "6px 12px", borderRadius: 999,
              fontSize: 11, fontWeight: 600, border: "none",
              background: r === range ? "var(--accent-green)" : "transparent",
              color: r === range ? "oklch(0.15 0.03 155)" : "var(--text-secondary)",
...
// ── HeroCard ───────────────────────────────────────────────────
function HeroCard({ label, value, delta }: { label: string; value: string; delta?: { value: string; positive: boolean } }) {
  return (
    <div style={{
      ...
      <div className="hero-num" style={{
        fontSize: "clamp(2.25rem, 4.5vw, 3rem)", fontWeight: 700, marginTop: 8,
      ...
      {delta && (
        <span className="pill" style={{
          background: delta.positive ? "var(--accent-green-soft)" : "color-mix(in oklch, var(--status-danger) 15%, transparent)",
          color: delta.positive ? "var(--accent-green)" : "var(--status-danger)",
          fontSize: 11, fontWeight: 700, padding: "3px 9px", marginTop: 8,
        }}>
          {delta.value}
        </span>
      )}
...
function AylikTab({ range }: { range: Range }) {
  ...
  const monthsCount = range === "Bu Ay" ? 1 : range === "Geçen Ay" ? 2 : range === "3 Ay" ? 3 : range === "6 Ay" ? 6 : 12;
  ...
  const incomeSeries = xLabels.map((_, i) => ({
    x: i,
    y: totalIncome > 0 ? totalIncome * (0.85 + Math.sin(i) * 0.1 + i * 0.02) : 0,
  }));
  const expenseSeries = xLabels.map((_, i) => ({
    x: i,
    y: totalExpense > 0 ? totalExpense * (0.9 + Math.cos(i) * 0.08 + i * 0.015) : 0,
  }));
  ...
  const deltaPct = prevExpense > 0 ? ((lastExpense - prevExpense) / prevExpense) * 100 : 0;

  // Top categories
  const byCategory = useMemo(() => {
    const map = new Map<string, number>();
    budgetData.expenses.forEach((e) => map.set(e.category, (map.get(e.category) ?? 0) + e.amount));
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{
        ...
        <div style={{ overflowX: "auto" }}>
          <LineAreaChart
            ...
            height={280}
            series={[
              { color: "var(--accent-green)", data: incomeSeries,  label: "Gelir" },
              { color: "var(--status-danger)", data: expenseSeries, label: "Gider" },
            ]}
            xLabels={xLabels}
          />
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16 }}>
        <DeltaCard label="EN ÇOK ARTAN KATEGORİ" cat={topCat?.[0]} amount={topCat?.[1] ?? 0} delta="↑ %12" positive={false} />
        <DeltaCard label="EN AZ HARCAMA"          cat={lowCat?.[0]} amount={lowCat?.[1] ?? 0} delta="↓ %8"  positive={true} />
      </div>
      ...
      <div style={{
        ...
        <div className="section-label" style={{ marginBottom: 16 }}>TOP 5 KATEGORİ</div>
        <BarChart
          height={280}
          groups={byCategory.slice(0, 5).map(([cat, val]) => {
            ...
            return {
              label: meta.name,
              bars: [{ color: meta.colorVar, value: val, max: byCategory[0]?.[1] ?? 1 }],
            };
          })}
        />
      </div>
...
function DeltaCard({ label, cat, amount, delta, positive }: { label: string; cat?: string; amount: number; delta: string; positive: boolean }) {
  return (
    <div style={{
      background: "var(--bg-surface)", borderRadius: "var(--r-lg)", boxShadow: "var(--shadow-card)",
      ...
      {cat ? (
        ...
        <div style={{ fontSize: 13, color: "var(--text-tertiary)" }}>Veri yok</div>
      )}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
        <span className="hero-num" style={{ fontSize: 22, fontWeight: 700, color: "var(--text-primary)" }}>
          {formatMoney(amount)}
        </span>
        <span className="pill" style={{
          background: positive ? "var(--accent-green-soft)" : "color-mix(in oklch, var(--status-danger) 15%, transparent)",
          color: positive ? "var(--accent-green)" : "var(--status-danger)",
          fontSize: 11, fontWeight: 700, padding: "3px 8px",
          display: "inline-flex", alignItems: "center", gap: 3,
        }}>
          {positive ? <ArrowDown style={{ width: 11, height: 11 }} /> : <ArrowUp style={{ width: 11, height: 11 }} />}
          {delta}
        </span>
      </div>
    </div>
...
function AnalitikTab({ range }: { range: Range }) {
  ...
  const totalSavings = (budgetData.savingsGoals || []).reduce((s, g) => s + g.currentAmount, 0);
  const savingsSeries = xLabels.map((_, i) => ({ x: i, y: (totalSavings / months) * (i + 1) }));

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <HeroCard label="SEÇİLEN ARALIKTA TOPLAM HARCAMA" value={formatMoney(totalSpent)} />
      ...
      <div style={{
        ...
        <div className="section-label" style={{ gridColumn: "1 / -1", marginBottom: 4 }}>KATEGORİ DAĞILIMI</div>
        <DonutChart
          slices={slices.length > 0 ? slices : [{ value: 1, color: "var(--bg-tint)", label: "—" }]}
          size={200}
          centerText={
            <div>
              ...
              <div className="hero-num" style={{ fontSize: 18, fontWeight: 700, color: "var(--text-primary)", marginTop: 4 }}>
            ...
            top5.map(([cat, val], i) => {
              ...
              return (
                <div key={cat} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <div style={{ flex: 1, height: 6, background: "var(--bg-tint)", borderRadius: 999, overflow: "hidden" }}>
                    <div style={{ width: `${pct}%`, height: "100%", background: meta.colorVar, borderRadius: 999 }} />
                  </div>
                  <span className="hero-num" style={{ fontSize: 12, fontWeight: 700, color: "var(--text-primary)", whiteSpace: "nowrap" }}>
                    {formatMoney(val)}
                  </span>
                </div>
              );
      ...
      <div style={{
        ...
        <div className="section-label" style={{ marginBottom: 16 }}>NET TASARRUF TRENDİ</div>
        <div style={{ overflowX: "auto" }}>
          <LineAreaChart
            width={Math.max(640, months * 80)}
            height={240}
            series={[{ color: "var(--owner-yigit)", data: savingsSeries, label: "Birikim" }]}
            xLabels={xLabels}
          />
        </div>
      </div>
      ...
      <div style={{
        ...
        <div className="section-label" style={{ marginBottom: 16 }}>AYLIK KATEGORİ DAĞILIMI</div>
        <BarChart
          height={260}
          groups={top5.map(([cat, val]) => {
            ...
            return {
              label: meta.name,
              bars: [{ color: meta.colorVar, value: val, max: top5[0]?.[1] ?? 1 }],
            };
          })}
        />
      </div>
