...
function OwlSvg({ s }: { s: number }) {
  return (
    <svg viewBox="0 0 100 100" width={s} height={s} style={{ display: 'block' }}>
      ...
      <path d="M 46 52 L 54 52 L 50 60 Z" fill="oklch(0.72 0.16 65)" stroke="oklch(0.45 0.12 60)" strokeWidth="1" />
      <path d="M 18 60 Q 22 76, 32 84" stroke="oklch(0.35 0.08 250)" strokeWidth="2.5" fill="none" strokeLinecap="round" />
      <path d="M 82 60 Q 78 76, 68 84" stroke="oklch(0.35 0.08 250)" strokeWidth="2.5" fill="none" strokeLinecap="round" />
    </svg>
  );
}
...
function PandaSvg({ s }: { s: number }) {
  return (
    <svg viewBox="0 0 100 100" width={s} height={s} style={{ display: 'block' }}>
      ...
      <ellipse cx="36" cy="48" rx="9" ry="11" fill="oklch(0.20 0.02 300)" transform="rotate(-18 36 48)" />
      <ellipse cx="64" cy="48" rx="9" ry="11" fill="oklch(0.20 0.02 300)" transform="rotate(18 64 48)" />
      ...
      <path d="M 50 65 L 50 69 M 50 69 Q 46 72, 44 70 M 50 69 Q 54 72, 56 70" stroke="oklch(0.20 0.02 300)" strokeWidth="1.5" fill="none" strokeLinecap="round" />
...
const OWNER_META: Record<AvatarWho, { color: string; soft: string; kind: 'owl' | 'panda' | 'glyph'; glyph?: string }> = {
  yigit: { color: 'var(--owner-yigit)', soft: 'var(--owner-yigit-soft)', kind: 'owl' },
  arzu:  { color: 'var(--owner-arzu)',  soft: 'var(--owner-arzu-soft)',  kind: 'panda' },
  ev:    { color: 'var(--owner-ev)',    soft: 'var(--owner-ev-soft)',    kind: 'glyph', glyph: '🏠' },
  tumu:  { color: 'var(--owner-tumu)', soft: 'var(--owner-tumu-soft)', kind: 'glyph', glyph: '★' },
};
...
export function Avatar({ who, size = 32 }: AvatarProps) {
  ...
  return (
    ...
    >
      {m.kind === 'owl'   ? <OwlSvg s={size * 0.92} />
       : m.kind === 'panda' ? <PandaSvg s={size * 0.92} />
       : m.glyph}
    </div>
  );
}
