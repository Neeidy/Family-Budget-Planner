...
export function OwnerCard({ who, title, amount, subtitle, cats = [] }: OwnerCardProps) {
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span className="section-label" style={{ fontSize: 10 }}>{title}</span>
      </div>
      <div
        className="hero-num"
        style={{ fontSize: 38, fontWeight: 700, marginTop: 12, letterSpacing: '-0.025em' }}
      >
        {amount}
      </div>
      {subtitle && (
