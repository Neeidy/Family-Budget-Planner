...
export function DialogShell({ open, onClose, title, width = 480, children, footer }: DialogShellProps) {
  useEffect(() => {
    if (!open) return;
    ...
    >
      <div
        ...
        style={{
          ...
          borderRadius: 24,
          boxShadow: "0 24px 64px -12px rgba(0,0,0,0.7), 0 0 0 1px var(--border-faint)",
          overflow: "hidden",
          maxHeight: "90vh",
      ...
      >
        <div style={{
          ...
          <div id="dialog-title" style={{ fontSize: 17, fontWeight: 700, letterSpacing: "-0.01em", color: "var(--text-primary)" }}>{title}</div>
          <button
            type="button"
            onClick={onClose}
            aria-label="Kapat"
...
export function TextInput({ value, onChange, placeholder, prefix, type = "text", step, min, max, autoFocus }: TextInputProps) {
  const id = useId();
  return (
    <div style={{
      ...
      {prefix && <span style={{ color: "var(--text-tertiary)", fontWeight: 600 }}>{prefix}</span>}
      <input
        id={id}
        type={type}
        step={step}
...
export function RadioRow<T extends string | number>({ value, options, onChange }: {
  value: T;
