 ...
 */
export function TodaySummaryStripCompact({ mobile, filter }: TodaySummaryStripCompactProps) {
  const upcoming = filter === "tumu"
    ? SAMPLE_UPCOMING
    : SAMPLE_UPCOMING.filter((u) => u.who === filter);
  const totalDue = upcoming.reduce((s, u) => s + u.amount, 0);
  const ownerName = ({ yigit: "Yigit", arzu: "Arzu", ev: "Ev", tumu: "Hepsi" } as const)[filter];

  return (
    <div className="card lift" style={{ position: "relative", overflow: "hidden", padding: mobile ? 20 : 24 }}>
      <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", flexWrap: "wrap", gap: 6 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
          <span className="section-label" style={{ margin: 0 }}>YARIN ÖDENECEK</span>
          <span style={{
            fontSize: 9, fontWeight: 700, padding: "2px 7px", borderRadius: 999,
            background: "color-mix(in oklch, var(--status-warning) 18%, transparent)",
            color: "var(--status-warning)", flexShrink: 0,
          }}>{upcoming.length}</span>
        </div>
        <span className="tnum" style={{ fontSize: 14, fontWeight: 700, color: "var(--status-warning)", flexShrink: 0 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 12 }}>
        {upcoming.length === 0 && (
          ...
          }}>Bu profilde yaklaşan ödeme yok ✓</div>
        )}
        {upcoming.slice(0, 4).map((u, i) => (
          <div key={i} style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 10, color: "var(--text-tertiary)" }}>{u.days} gün sonra</div>
            </div>
            <Avatar who={u.who} size={20}/>
            <div className="tnum" style={{ fontSize: 13, fontWeight: 700, color: "var(--status-warning)", flexShrink: 0 }}>
