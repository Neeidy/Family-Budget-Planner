...
describe('computeRollover', () => {
  it('clears incomes regardless of content', () => {
    const state = makeState({ incomes: [{ id: '1', name: 'Maaş', amount: 5000 }] });
  ...
  it('carries forward only type=Sabit expenses', () => {
    const state = makeState({
      expenses: [
        { id: 'e1', type: 'Sabit', status: 'Odendi', category: 'Kira', amount: 1200 },
        { id: 'e2', type: 'Degisken', status: 'Odendi', category: 'Market', amount: 300 },
        { id: 'e3', type: 'Borc', status: 'Odendi', category: 'Kredi', amount: 500 },
        { id: 'e4', type: 'Birikim', status: 'Odendi', category: 'Birikim', amount: 200 },
      ],
    });
    const result = computeRollover(state, deterministicId);
  ...
  it('assigns new id to each carried Sabit expense', () => {
    const state = makeState({
      expenses: [
        { id: 'old-1', type: 'Sabit', status: 'Odendi', category: 'Kira', amount: 1200 },
        { id: 'old-2', type: 'Sabit', status: 'Gecikti', category: 'İnternet', amount: 50 },
      ],
    });
    const result = computeRollover(state, deterministicId);
