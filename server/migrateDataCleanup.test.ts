...
import { migrateIncome, migrateExpense, migrateBudgetLimit, migrateData } from '../shared/migrateLogic';

// ─── migrateIncome ───────────────────────────────────────────────────────────

describe('migrateIncome', () => {
  it('converts planned+actual to amount (uses actual)', () => {
    const result = migrateIncome({ id: '1', name: 'Maaş', planned: 4000, actual: 3800, owner: 'Benim' });
    expect(result).toEqual({ id: '1', name: 'Maaş', amount: 3800, owner: 'Benim' });
  ...
  it('is idempotent: migrating twice gives same result', () => {
    const old = { id: '1', planned: 3000, actual: 2800, name: 'X', owner: 'Benim' };
...
describe('migrateExpense', () => {
  it('converts planned+actual to amount and removes urgency', () => {
    const input = {
      id: 'e1', category: 'Kira', type: 'Sabit',
      planned: 1500, actual: 1500,
      urgency: 'Zorunlu', status: 'Odendi', owner: 'Ev',
    };
  ...
  it('is a no-op when already migrated (amount, no planned/actual/urgency)', () => {
    const already = { id: 'e1', amount: 200, category: 'Market', type: 'Degisken' };
    const result = migrateExpense(already);
    expect(result).toBe(already);
  });

  it('migrates expense that only has urgency removed (no planned/actual)', () => {
    const input = { id: 'e1', amount: 100, urgency: 'Esnek', category: 'Market' };
    const result = migrateExpense(input);
    expect('urgency' in result).toBe(false);
  ...
  it('is idempotent: migrating twice gives same result', () => {
    const old = { id: 'e1', planned: 200, actual: 180, urgency: 'Zorunlu', category: 'Fatura' };
...
describe('migrateBudgetLimit', () => {
  it("maps 'Eşim' → 'Esim'", () => {
    const result = migrateBudgetLimit({ id: 'bl1', category: 'Market', limit: 400, owner: 'Eşim' });
    expect(result.owner).toBe('Esim');
  });

  it("maps 'Ortak' → 'Ev'", () => {
    const result = migrateBudgetLimit({ id: 'bl1', category: 'Kira', limit: 1500, owner: 'Ortak' });
    expect(result.owner).toBe('Ev');
  });

  it("leaves 'Benim' unchanged", () => {
    const result = migrateBudgetLimit({ id: 'bl1', category: 'Eğitim', limit: 200, owner: 'Benim' });
    expect(result.owner).toBe('Benim');
  });

  it("leaves 'Esim' unchanged (already migrated)", () => {
    const result = migrateBudgetLimit({ id: 'bl1', owner: 'Esim', limit: 100, category: 'X' });
    expect(result.owner).toBe('Esim');
  });

  it("leaves 'Ev' unchanged (already migrated)", () => {
    const result = migrateBudgetLimit({ id: 'bl1', owner: 'Ev', limit: 100, category: 'X' });
    expect(result.owner).toBe('Ev');
  });
...
describe('migrateData', () => {
  const OLD_FIXTURE = {
    incomes: JSON.stringify([
      { id: 'i1', name: 'Maaş', planned: 5000, actual: 4800, owner: 'Benim', date: '2026-04-01', notes: '' },
    ]),
    expenses: JSON.stringify([
      {
        id: 'e1', category: 'Kira', subcategory: 'Ev', type: 'Sabit',
        planned: 1500, actual: 1500, urgency: 'Zorunlu',
        status: 'Odendi', owner: 'Ev', notes: '', paymentDay: '1',
      },
      {
        id: 'e2', category: 'Market', subcategory: 'Gıda', type: 'Degisken',
        planned: 300, actual: 280, urgency: 'Esnek',
        status: 'Odendi', owner: 'Benim', notes: '', paymentDay: '',
      },
    ]),
  ...
  it('reports changed=false when data is already clean', () => {
    const clean = {
      incomes: JSON.stringify([{ id: 'i1', amount: 5000, owner: 'Benim', name: 'Maaş', date: '', notes: '' }]),
      expenses: JSON.stringify([{ id: 'e1', amount: 1500, category: 'Kira', type: 'Sabit', status: 'Odendi', owner: 'Ev', notes: '', paymentDay: '', subcategory: '' }]),
      budgetLimits: JSON.stringify([{ id: 'bl1', category: 'Market', limit: 400, owner: 'Esim' }]),
      debts: '[]',
      savingsGoals: '[]',
      annualPayments: '[]',
      installments: '[]',
  ...
  it('handles empty arrays without error', () => {
    const empty = {
      incomes: '[]', expenses: '[]', budgetLimits: '[]',
      debts: '[]', savingsGoals: '[]', annualPayments: '[]', installments: '[]',
    };
